...
import * as ConversationApi from '../../ApiRedux/api/ConversationRequest';
...
export default function Messagerie() {
 ...
 console.log(users);
  ...
  // const [usersdata, setUsersdata] = useState(users?.filter((item) => item.id !== user.id));
...
// get conversation of two users
  ...
  useEffect(() => {
    const getConversations = async () => {
      try {
        // const res = await axios.get("http://localhost:8088/api/conversations/" + user.id);
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        // const res = await axios.get(`http://localhost:8088/api/messages/${currentChat.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // const res = await axios.post("http://localhost:8088/api/messages", message);
